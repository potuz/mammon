...
Container::serialize_(const std::vector<const Container *> &parts) {
  ...
  for (auto *part : parts) {
    ...
    if (part->get_ssz_size() == 0) {
      ...
      variable_part.insert(variable_part.end(), part_ssz.begin(),
                           part_ssz.end());
    } else {
    ...
    if (part_size) {
      ...
      if (last_offset) {
        if (current_offset < last_offset)
          return false;
        if (!last_variable_part->deserialize(begin + last_offset,
...
bool Container::decode_(const YAML::Node &node, std::vector<Part> parts) {
  return std::all_of(parts.begin(), parts.end(), [&node](Part part) {
    return part.second->decode(node[part.first]);
  });
}
